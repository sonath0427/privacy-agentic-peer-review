...
Usage (run from repo root):
    ...
    python src/data_collection/fetch_openreview_data.py --venue NeurIPS --year 2024 --outdir data/raw/neurips2024 --download-pdfs
...
# ---- Venue ID mapping (API v2 group ids) ----
# NOTE: verify these against https://openreview.net for the exact year you need
# — venue ids can change slightly year to year.
VENUE_IDS = {
...
def fetch_all_submissions(client, venue_id):
    """Fetch ALL submissions (accepted + rejected + withdrawn) for a venue."""
...
def _extract_value(field):
    """OpenReview v2 wraps values as {'value': ...}. Unwrap safely."""
...
def parse_note_to_record(note, venue_name, year, tier="A*"):
    ...
    return {
        ...
        "collector": "Sonath",
        "raw_file": {
            "pdf_path": None,   # filled in after PDF download
            "file_type": "pdf",
            "has_reviews": len(reviews) > 0
        },
        "parsed": None,  # filled in later by the Marker/Grobid parsing step
...
def main():
    ...
    parser.add_argument("--limit", type=int, default=None,
                         help="Only process first N papers — useful for a quick test run")
